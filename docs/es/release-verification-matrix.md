...
| Windows — instalación limpia | Ningún runner de CI instala de verdad y la reputación de SmartScreen depende de telemetría de base instalada que un runner no produce.                                                                          | Instalar primero el EXE NSIS: CTA, progreso, apertura, actualización, uninstall y `/S`, sin UAC; comprobar que datos sobreviven. El MSI es alternativo y conserva el checkbox `.cslmap` desmarcado.                      |
