...
| Windows — clean install | No CI runner actually installs, and SmartScreen reputation depends on install-base telemetry no runner produces.                                                                                                   | Install the NSIS EXE first: CTA, progress, launch, update, uninstall, and `/S`, without UAC; confirm user data survives. MSI is the alternative and keeps the `.cslmap` checkbox unticked.                   |
