...
describe('buildTransitRenderData — connectors and stations', () => {
  ...
  it('includes a confirmed transfer (≥2 modes) in transferMarkers', () => {
    ...
    const data = buildTransitRenderData(city);
    expect(data.transferMarkers.features).toHaveLength(1);
  ...
  it('never fabricates any candidate for a city with no transit at all', () => {
    const city = makeCityData({
      roadNodes: [NODE_A, NODE_B],
      roadSegments: [SEG_1],
      transitLines: [],
    ...
    expect(data.connectors.features).toEqual([]);
    expect(data.stations.features).toEqual([]);
    expect(data.stationDots.features).toEqual([]);
    expect(data.transferMarkers.features).toEqual([]);
  });
});
